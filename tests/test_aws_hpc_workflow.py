...
class AwsHpcWorkflowTest(unittest.TestCase):
    ...
    def test_family_prefix_parses_common_aws_shapes(self) -> None:
        ...
        for instance_type, expected in cases.items():
            self.assertEqual(aws_hpc_workflow.family_prefix(instance_type), expected)
    ...
    def test_linux_user_from_iam_user_is_deterministic(self) -> None:
        ...
        for iam_user, expected in cases.items():
            self.assertEqual(aws_hpc_workflow.linux_user_from_iam_user(iam_user), expected)
    ...
    def test_compute_resource_name_distinguishes_tiny_memory_shapes(self) -> None:
        ...
        self.assertNotEqual(
            aws_hpc_workflow.compute_resource_name("cpu", 1, 512, 0),
            aws_hpc_workflow.compute_resource_name("cpu", 1, 627, 0),
        )

    def test_manifest_is_parallelcluster_first(self) -> None:
        """The manifest captures the requested ParallelCluster decision."""
        manifest = aws_hpc_workflow.load_manifest(
            Path("config/aws_hpc_workflow_manifest.json")
        )
        ...
        self.assertEqual(manifest["parallelcluster"]["cluster_name"], "wanglab-hpc")
        self.assertEqual(
    ...
    def test_cluster_architecture_override_selects_matching_head_node(self) -> None:
        ...
        arm_manifest = aws_hpc_workflow.manifest_with_cluster_architecture(
            manifest, "arm64"
        )
        self.assertEqual(aws_hpc_workflow.head_node_instance_type(manifest), "r6a.xlarge")
        self.assertEqual(arm_manifest["ec2_selection"]["cluster_architecture"], "arm64")
        self.assertEqual(aws_hpc_workflow.head_node_instance_type(arm_manifest), "r7g.xlarge")
    ...
    def test_selected_users_include_lab_members_and_admins_without_diego(self) -> None:
        ...
        with mock.patch.object(aws_hpc_workflow, "run_json", side_effect=fake_run_json):
            users = aws_hpc_workflow.lab_users_from_iam(manifest)
    ...
    def test_generated_cluster_config_contains_core_shape(self) -> None:
        ...
        with mock.patch.object(aws_hpc_workflow, "lab_users_from_iam", return_value=fake_users):
            with mock.patch.object(aws_hpc_workflow, "grouped_candidates", return_value=fake_groups):
                config = aws_hpc_workflow.generated_cluster_config(manifest)
        ...
        self.assertEqual(
            ...
            [
                ...
                "s3://wanglab-misc-backups/parallelcluster-assets/bootstrap/parallelcluster_active_instance_types.txt",
            ],
        )
        self.assertEqual(
    ...
    def test_generated_cluster_config_uses_arm_head_when_requested(self) -> None:
        ...
        manifest = aws_hpc_workflow.manifest_with_cluster_architecture(
            aws_hpc_workflow.load_manifest(Path("config/aws_hpc_workflow_manifest.json")),
            "arm64",
        )
        fake_users = [
    ...
    def test_generic_catalog_includes_arm_and_previous_generation_tiny_instances(self) -> None:
        ...
        with mock.patch.object(aws_hpc_workflow, "ec2_instance_type_rows", return_value=fake_rows):
            with mock.patch.object(aws_hpc_workflow, "available_instance_types", return_value=fake_offered):
                with mock.patch.object(aws_hpc_workflow, "linux_on_demand_prices", return_value=fake_prices):
                    full_rows = aws_hpc_workflow.generic_candidate_rows(manifest, False)
                    active_rows = aws_hpc_workflow.generic_candidate_rows(manifest, True)
                    catalog_rows = aws_hpc_workflow.instance_catalog_rows(manifest, False)
    ...
    def test_bootstrap_script_has_aggregate_limits_and_job_submit_policy(self) -> None:
        ...
        self.assertIn("wanglab-sbatch", text)
    ...
    def test_pcluster_create_command_has_dryrun(self) -> None:
        ...
        command = aws_hpc_workflow.pcluster_create_command(manifest, True)
        self.assertEqual(command[0:2], ["pcluster", "create-cluster"])
        self.assertIn("--dryrun", command)
        self.assertIn("true", command)
        self.assertIn("config/parallelcluster_wanglab.yaml", command)
    ...
    def test_pcluster_delete_command_targets_cluster(self) -> None:
        ...
        command = aws_hpc_workflow.pcluster_delete_command(manifest)
        self.assertEqual(command[0:2], ["pcluster", "delete-cluster"])
        self.assertIn("wanglab-hpc", command)
    ...
    def test_bootstrap_upload_command_uses_assets_bucket(self) -> None:
        ...
        command = aws_hpc_workflow.bootstrap_upload_command(manifest)
        self.assertEqual(command[0:3], ["aws", "s3", "cp"])
        self.assertEqual(command[3], "scripts/parallelcluster_create_lab_users.sh")
        self.assertEqual(
            command[4],
            "s3://wanglab-misc-backups/parallelcluster-assets/bootstrap/parallelcluster_create_lab_users.sh",
        )
    ...
    def test_active_instance_types_upload_command_uses_assets_bucket(self) -> None:
        ...
        command = aws_hpc_workflow.active_instance_types_upload_command(manifest)
        self.assertEqual(command[0:3], ["aws", "s3", "cp"])
    ...
    def test_slurm_submit_commands_filter_gpu_and_expected_failures(self) -> None:
        ...
        default_commands = aws_hpc_workflow.slurm_submit_commands(manifest, False, False)
        gpu_commands = aws_hpc_workflow.slurm_submit_commands(manifest, True, False)
        all_commands = aws_hpc_workflow.slurm_submit_commands(manifest, True, True)
